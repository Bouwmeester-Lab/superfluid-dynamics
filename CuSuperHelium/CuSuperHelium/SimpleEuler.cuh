...
template<int N, size_t batchSize>
void Euler<N, batchSize>::doTimeStep()
{
	...
	cublasZaxpy(handle, batchSize * N, &time_step, timeStepManager.devVelocitiesLower, 1, devZ, 1); // here devZ get's overwritten with the new positions
	...
	cublasZaxpy(handle, batchSize * N, &time_step, timeStepManager.devRhsPhi, 1, devPhi, 1); // update Phi with the RHS of the phi equation
}
