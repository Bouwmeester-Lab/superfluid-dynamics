...
class TimedBoundaryIntegralCalculator : public BaseBoundaryIntegralCalculator<N, batchSize>
{
...
public:
	TimedBoundaryIntegralCalculator(ProblemProperties& problemProperties, TimedBoundaryProblem<N, batchSize>& boundaryProblem) : BaseBoundaryIntegralCalculator<N, batchSize>(problemProperties, boundaryProblem), timedBoundaryProblem(boundaryProblem)
	{
	}
	virtual ~TimedBoundaryIntegralCalculator() {}

	virtual void calculateRhs(const ProblemPointers& problemPointers, const std_complex* initialState, std_complex* rhs) override
	{
		BaseBoundaryIntegralCalculator<N, batchSize>::calculateRhs(initialState, rhs); // Call the base class implementation to calculate the standard rhs
		// Now calculate the time-dependent part of the rhs using the boundary problem's method
		timedBoundaryProblem.CalculateTimeDependentRhsPhi(problemPointers, rhs, this->problemProperties);
	}
