...
/// <param name="n"></param>
static __global__ void vector_subtract_complex_real(const cufftDoubleComplex* a, const double* b, cufftDoubleComplex* out, int n);
/// <summary>
...
/// <param name="out"></param>
/// <param name="n"></param>
static __global__ void vector_scalar_add_complex_real(const cufftDoubleComplex* a, const double b, cufftDoubleComplex* out, int n, int start);

static __global__ void cotangent_complex(const cufftDoubleComplex* a, cufftDoubleComplex* out, int n);
static __global__ void real_to_complex(const double* x, cuDoubleComplex* x_c, int N);
static __global__ void complex_to_real(const cuDoubleComplex* x_c, double* x, int N);
static __global__ void conjugate_vector(cuDoubleComplex* x, cuDoubleComplex* z, int N);

static __global__ void add_k_vectors(cufftDoubleComplex* k1, cufftDoubleComplex* k2, cufftDoubleComplex* k3, cufftDoubleComplex* k4, cufftDoubleComplex* result, int n) {
...
static __global__ void add_k_vectors(std_complex* k1, std_complex* k2, std_complex* k3, std_complex* k4, std_complex* result, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
...
static __global__ void linspace(double start, double end, double* out, int steps)
{
...
static __global__ void vector_subtract_complex_real(const std_complex* a, const double* b, std_complex* out, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        out[i] = a[i] - b[i];
    }
...
static __global__ void batched_vector_subtract_singletime_complex_real(const std_complex* a, const double* b, std_complex* out, int n, size_t batchSize) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
static __global__ void vector_scalar_add_complex_real(const std_complex* a, const double b, std_complex* out, int n, int start)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
...
static __global__ void vector_mutiply_scalar(const std_complex* a, const double b, std_complex* out, int n, int start)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
...
static __global__ void real_to_complex(const double* x, std_complex* x_c, int N) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < N)
...
static __global__ void complex_to_real(const std_complex* x_c, double* x, int N) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < N)
...
static __global__ void conjugate_vector(std_complex* x, std_complex* z, int N)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < N) {
